...
def energy(strip, color):
    for i in range(int(LED_COUNT/2)):
        ...
        time.sleep(0.006)
    for i in range(int(LED_COUNT/2)):
        strip.setPixelColor(int(LED_COUNT/2) - (int(LED_COUNT/2) -i), Color(0,0,0))
        strip.setPixelColor(int(LED_COUNT/2) + (int(LED_COUNT/2) -i), Color(0,0,0))
        strip.show()
        time.sleep(0.006)

def energyInverse(strip, color):
    for i in range(int(LED_COUNT/2)):
        strip.setPixelColor(LED_COUNT - i, color)
        strip.setPixelColor(i, color)
        strip.show()
        time.sleep(0.07)
    color = Color(randrange(170), randrange(170), randrange(170))
    for i in range(int(LED_COUNT/2)):
        strip.setPixelColor(int(LED_COUNT/2)-i, color)
        strip.setPixelColor((int(LED_COUNT/2) +i), color)
        strip.show()
...
if __name__ == '__main__':
    ...
    COLOR = Color(0, 187, 0)
    COLOR_1 = Color(255, 187, 0)
    COLOR_2 = Color(0, 0, 255)
    while True:
        energyInverse(strip, Color(randrange(170), randrange(170), randrange(170)))
        energyInverse(strip, Color(randrange(170), randrange(170), randrange(170)))
        energyInverse(strip, Color(randrange(170), randrange(170), randrange(170)))
